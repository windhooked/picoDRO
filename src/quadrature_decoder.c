...
{
    if (!pio_can_add_program(pio, &quadrature_decoder_program))
    {
        return false;
    }
    qd->m_pio = pio;
    ...
    pio_add_program(qd->m_pio, &quadrature_decoder_program);
...
int32_t add_quadrature_decoder(quadrature_decoder* qd, uint32_t pinBase)
{
    ...
    pio_sm_config smConfig = quadrature_decoder_program_get_default_config(programOffset);
    ...
    pio_sm_init(qd->m_pio, stateMachine, quadrature_decoder_offset_start, &smConfig);
